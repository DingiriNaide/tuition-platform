...
export default function ProgressReportCreate({
    booking,
    gradeOptions,
    defaults,
}: Props) {
    const { data, setData, post, processing, errors, transform } = useForm<ReportForm>({
        ...
        total_sessions:        String(defaults.total_sessions),
        attended_sessions:     String(defaults.attended_sessions),
        absent_sessions:       String(defaults.absent_sessions),
        late_sessions:         String(defaults.late_sessions),
        is_published:          false,
    });
